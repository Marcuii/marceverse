/**
 * @file Contact.jsx
 * @description Contact form section powered by EmailJS.
 *
 * Sends emails directly from the browser without a backend relay.
 * Also displays social links and contact info from GeneralInfo context.
 */
...
export default function Footer() {
  ...
  const { generalInfo, loading: infoLoading } = useGeneralInfo();
  ...
  if (infoLoading || !generalInfo) return null;
  ...
  return (
    ...
    >
      ...
      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12">
        ...
        <div className="flex flex-col items-start space-y-4">
          ...
          <div className="flex flex-wrap gap-4 text-4xl">
            {generalInfo.socials.map((social, index) => (
              <a
                key={index}
                href={social.url}
                target="_blank"
              ...
              >
                {iconMap[social.icon] || <span>{social.platform[0]}</span>}
              </a>
            ))}
          </div>
        </div>
      ...
      <div className="text-center text-sm text-neutral-content mt-12">
        © {new Date().getFullYear()} {generalInfo.name}. All rights reserved.
      </div>
    </section>
  );
}
